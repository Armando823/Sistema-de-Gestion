...
function App() {
  ...
  async function updatePhotos(id, event) {
    ...
    try {
      ...
      setRepairs(repairs.map((repair) => repair.id === id ? { ...repair, photos, updated: 'Ahora' } : repair));
  ...
  return <div className="app-shell"><header className="topbar"><div><span className="eyebrow">TALLER DIGITAL</span><h1>Control de reparaciones</h1></div><nav><button className={view === 'admin' ? 'nav-button active' : 'nav-button'} onClick={() => setView('admin')}>Administrador</button><button className={view === 'client' ? 'nav-button active' : 'nav-button'} onClick={() => setView('client')}>Consulta cliente</button></nav></header>{notice && <div className="notice" role="status">{notice}<button onClick={() => setNotice('')}>Cerrar</button></div>}{view === 'admin' ? <AdminView form={form} setForm={setForm} formError={formError} addRepair={addRepair} addPhotos={addPhotos} search={search} setSearch={setSearch} repairs={repairs} filteredRepairs={filteredRepairs} updateStatus={updateStatus} updatePhotos={updatePhotos} /> : <ClientView repairs={repairs} />}</div>;
}

function AdminView({ form, setForm, formError, addRepair, addPhotos, search, setSearch, repairs, filteredRepairs, updateStatus, updatePhotos }) {
  return <main className="content"><section className="intro"><div><p className="eyebrow">ÁREA DEL ADMINISTRADOR</p><h2>Las reparaciones, claras de un vistazo.</h2><p>Solo el administrador registra órdenes, evidencia el estado del equipo y conserva la autorización del cliente.</p></div><div className="summary"><strong>{repairs.length}</strong><span>órdenes activas</span></div></section><section className="workspace"><RepairForm form={form} setForm={setForm} formError={formError} addRepair={addRepair} addPhotos={addPhotos} /><OrderList search={search} setSearch={setSearch} filteredRepairs={filteredRepairs} updateStatus={updateStatus} updatePhotos={updatePhotos} /></section></main>;
}
...
function ClientView({ repairs }) {
  ...
  return <main className="client-view"><div className="client-card"><p className="eyebrow">ÁREA DEL CLIENTE</p><h2>¿Dónde está tu equipo?</h2><p>El administrador crea la orden en el taller. Tú solo necesitas ingresar el código recibido para consultar su estado.</p><form onSubmit={findRepair} className="lookup"><input required maxLength="12" value={code} onChange={(event) => setCode(event.target.value)} placeholder="Ej. REP-1001" /><button className="primary-button">Consultar</button></form>{result && <div className="result"><span className="status-dot" /><div><strong>{result.status}</strong><p>{result.device}</p><small>Última actualización: {result.updated}</small>{result.photos?.length > 0 && <div className="client-photos"><strong>Fotos del equipo</strong><div className="photo-preview">{result.photos.map((photo, index) => <img key={photo} src={photo} alt={`Foto del equipo ${index + 1}`} />)}</div></div>}</div></div>}{result === false && <p className="error">No encontramos una orden con ese código.</p>}<p className="demo-hint">Prueba con <button onClick={() => setCode('REP-1001')}>REP-1001</button></p></div></main>;
}
