...
function App() {
  ...
  function addRepair(event) {
    ...
    if (!form.customer || !form.device || !form.problem) {
      setFormError('Completa cliente, equipo y falla reportada.');
      return;
    }
    setFormError('');
  ...
  return (
    <div className="app-shell">
      <header className="topbar"><div><span className="eyebrow">TALLER DIGITAL</span><h1>Control de reparaciones</h1></div><nav><button className={view === 'admin' ? 'nav-button active' : 'nav-button'} onClick={() => setView('admin')}>Panel técnico</button><button className={view === 'client' ? 'nav-button active' : 'nav-button'} onClick={() => setView('client')}>Consulta cliente</button></nav></header>
      {notice && <div className="notice" role="status">{notice}<button onClick={() => setNotice('')}>Cerrar</button></div>}
      {view === 'admin' ? <main className="content"><section className="intro"><div><p className="eyebrow">OPERACIÓN DIARIA</p><h2>Las reparaciones, claras de un vistazo.</h2><p>Registra equipos, actualiza su avance y comparte el código con cada cliente.</p></div><div className="summary"><strong>{repairs.length}</strong><span>órdenes activas</span></div></section><section className="workspace"><form className="panel form-panel" onSubmit={addRepair}><div className="panel-heading"><h3>Nueva reparación</h3><span>1</span></div><label>Cliente<input value={form.customer} onChange={(event) => setForm({ ...form, customer: event.target.value })} placeholder="Nombre completo" /></label><label>Teléfono<input value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="300 000 0000" /></label><label>Equipo<input value={form.device} onChange={(event) => setForm({ ...form, device: event.target.value })} placeholder="Marca y modelo" /></label><label>Falla reportada<textarea value={form.problem} onChange={(event) => setForm({ ...form, problem: event.target.value })} placeholder="Describe el problema" /></label>{formError && <p className="form-error" role="alert">{formError}</p>}<button className="primary-button">Crear orden</button></form><section className="panel orders-panel"><div className="panel-heading"><div><h3>Órdenes recientes</h3><p>{filteredRepairs.length} resultados</p></div><input className="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar orden o cliente" /></div><div className="order-list">{filteredRepairs.map((repair) => <article className="order" key={repair.id}><div className="order-id">{repair.id}<small>{repair.updated}</small></div><div><h4>{repair.device}</h4><p>{repair.customer} · {repair.problem}</p></div><select value={repair.status} onChange={(event) => updateStatus(repair.id, event.target.value)}>{repairStatuses.map((status) => <option key={status}>{status}</option>)}</select></article>)}{filteredRepairs.length === 0 && <p className="empty">No hay órdenes que coincidan.</p>}</div></section></section></main> : <ClientView repairs={repairs} />}
    </div>
  );
}
